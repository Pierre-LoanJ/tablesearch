...
module.exports = function(app, passport) {
    app.get('/profile/data/:column', isLoggedIn, function(req, res) {
        var column = req.params.column;
        column = column.replace(new RegExp("%20", 'g'), ' ')
        ...
        db.serialize(() => {
          db.all('SELECT "' + column + '", count(1) AS count, AVG(age) AS average FROM census_learn_sql GROUP BY "' + column + '"' + 'ORDER BY count(1) DESC', (err, rows) => {        
            if (err) {
    ...
    app.post('/profile/photo/upload', function(req, res){
      ...
      var id = req.session.passport.user;
        ...
        var filePath = form.uploadDir + "/profile_" + id + ".jpg"
        if (fs.existsSync(filePath)) {
            var imgData = fs.readFileSync(filePath, (error) => { console.error("erreur readFileSync") });
